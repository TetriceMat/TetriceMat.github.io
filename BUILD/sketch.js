...
function startBellMusic() {
  ...
  let notes = [660, 880, 740, 660];
  let delay = 0;
  for (let i = 0; i < notes.length; i++) {
    bellNoteTimeouts.push(setTimeout(() => {
      playBellNote(notes[i]);
    }, delay));
    delay += 700;
  }
}
...
function drawPetals(sway = 0) {
  ...
  for (let layer = 0; layer < 3; layer++) {
    ...
    let fillColor = layer === 0 ? color(255, 140, 0, alpha) : color(255, 190, 80, alpha);
    ...
    for (let i = 0; i < count; i++) {
      ...
      rotate(i * angleStep);
      translate(0, -radius);
      roughEllipse(0, 0, petalWidth, petalHeight, fillColor, alpha, 3);
      paintDabs(0, 0, petalWidth, petalHeight, 6, fillColor);
      pop();
    }
